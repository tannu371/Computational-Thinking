...
student = (name = "Tannu Kumari", kerberos_id = "tannu")
...
function get_reds(image::AbstractMatrix)
	# your code here!
	get_reds = fill(0.0, size(image)[1], size(image)[2])
	for i in 1:size(image)[1]
		for j in 1:size(image)[2]
			get_reds[i,j] = get_red(RGB(image[i,j].r,image[i,j].g,image[i,j].b))
		end
	end
	return get_reds
end

# ╔═╡ fa5c02c8-3e91-4917-9c27-511c21c013b5
md"""
get_reds = get_red.(image)
"""
...
# ╔═╡ 02450b0c-5825-492b-8639-0c4c49e08ee8
function get_greens(image)
	return get_green.(image)
end

# ╔═╡ c49437bb-aceb-4f0f-b5c0-3ec2f5588622
function get_blue(pixel)
	return pixel.b
end

# ╔═╡ 96caeadd-14c2-4821-a300-fe36ca7890e7
...
# ╔═╡ 726cf169-fe73-42ec-b95d-632a11d70507
function get_reds(image)
...
function mean_color(image)
	...
	no_of_pixels = image_size[1] * image_size[2]
	mean_red = sum(get_reds.(image)) / no_of_pixels
	mean_green = sum(get_greens.(image) / no_of_pixels)
	mean_blue = sum(get_blues.(image)) / no_of_pixels
...
philip_inverted = philip[ : , end:-1:1] # replace `missing` with your code!

# ╔═╡ d6fee5d6-120a-4d52-a59a-6b18c1873dfe
reverse(philip, dims=2)
...
function quantize(color::AbstractRGB)
	# your code here!
	quantized_red = floor(10 * color.r) / 10
	quantized_green = floor(10 * color.g) / 10
	quantized_blue = floor(10 * color.b) / 10
	return RGB(quantized_red, quantized_green, quantized_blue)
end
...
function noisify(x::Number, s)
	# your code here!
	return clamp(rand(collect(-s:0.001:s)), 0.0, 0.1)
end
...
@bind philip_noise Slider(0:0.01:0.6, show_value=true)
